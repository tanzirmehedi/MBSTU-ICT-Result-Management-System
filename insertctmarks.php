	...
	$session=$_POST['session'];	
		...
		$row=mysqli_fetch_array($sqq);
			...
			{				
				if($npassword==$row['password'] && $row['coursecode']==$code && $row['username']=$user_name)
				{
					...
					if ($result->num_rows > 0)
						...
						$sha=md5($user_name);
						$content="ctprimarystudentinfo.php?XsErCXz".$sha."@FCggVJyGYG%vvKI".$sha."Khv45747551".$sha;
			...
			else
			{
			  ...
			  $sha=md5($user_name);
			  $content="insertctmarks.php?xzBBcFXsErCXz".$sha."@FCggVJyGYG%vvKI".$sha."Khv45747551".$sha;
