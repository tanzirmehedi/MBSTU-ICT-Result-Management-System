...
if(isset($_POST['submit']))
{
	...
	if (!empty($code) && (!empty($npassword)) && (!empty($session)))
		{ 
		     ...
		     $sql="SELECT * FROM ctregistration WHERE coursecode='$code' AND session='$session'";
			 $result  = mysqli_query($dbh,$sql);
             $totalNumRowResult = mysqli_num_rows($result);
			  
             if($totalNumRowResult > 0)
			  {
              array_push($errors, "Subject already exist, please try new one.");
			  $sha=md5($user_name);
			  $content="registrationctmarks.php?BBcFXsErCXz".$sha."@FCggVJyGYG%vvKI".$sha."Khv45747551".$sha;
...
$sql="INSERT INTO ctregistration (username,password,session,coursecode,yearsem) VALUES('$user_name','$npassword','$session','$code','$yearsem')";
		  ...
		  if (mysqli_query($dbh, $sql))
			{
					...
					$sha=md5($user_name);
					  $content="insertctmarks.php?cTFDESBBcFXsErCXz".$sha."@FCggVJyGYG%vvKI".$sha."Khv45747551".$sha;
					  header('refresh:3;url='.$content);
			}
            ...
            <form  class="form-group" method="post" >
             ...
             <table class="table"> <tr class="active"><td class="success">&nbsp;</td></tr></table>
              ...
              <div class="form-group">
                <div class="col-sm-offset-0 col-sm-12">
                  <center>
                    <button type="submit" name="submit" class="btn btn-primary btn-labeled"> Registration <span class="btn-label btn-label-right"><i class="	fa fa-hand-o-up"></i></span></button>
                  </center><br>
                </div>
              </div>
            </form>
